...
use App\Rules\EmailExistsInProgram;
...
class ProgramApplicationImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        ...
        return [
            ImportColumn::make('program_id')
                ->requiredMapping()
                ->rules(['required', 'numeric', 'exists:programs,id']),
            ...
            ImportColumn::make('email')
                ...
                ->rules(['required', 'email', function ($attribute, $value, $fail) {
                    ...
                    if ($participant && ProgramApplication::where('program_id', $this->data['program_id'])
            ...
            ImportColumn::make('members_emails')
                ...
                ->nestedRecursiveRules(['required', 'email', function ($attribute, $value, $fail) {
                    ...
                    if ($participant && TeamMember::where('participant_id', $participant->id)
                        ->whereHas('team', fn($query) => $query->whereHas('application', fn($query) => $query->where('program_id', $this->data['program_id'])))
                        ->exists()) {
                        $fail("The participant is already in another team: " . $value);
                    }

//                    if (ProgramApplication::where('program_id', $this->data['program_id'])
...
//                    }
    ...
    public function resolveRecord(): ?ProgramApplication
    {
        ...
        $participant?->notify(new ProgramRegistration($application));
    ...
    public function getValidationRules(): array
    {
        return [
            ...
            'email' => ['required', 'email', new EmailExistsInProgram($this->data['program_id'])],
            'team_name' => ['max:255'],
...
//                }
    ...
    public function getValidationMessages(): array
    {
        return [
            ...
            'program_id.exists' => 'Please add a valid program Id.',
