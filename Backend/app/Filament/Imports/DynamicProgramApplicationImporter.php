...
class DynamicProgramApplicationImporter extends Importer
{
    ...
    {
        // Get only current program if it's open
        $currentProgramId = session('current_program_id');

        if (!$currentProgramId) {
            // If no current program, return empty examples
            $programIds = [];
            $formIdsArray = [];
            $openPrograms = collect();
        } else {
            // Get current program only if it's open
            $currentProgram = Program::published()
                ->active()
            ...
            if ($currentProgram) {
                // Get the form for this program
                $form = Form::where('program_id', $currentProgramId)
                    ->registrationType()
                    ->published()
                    ->active()
                    ->first();
                ...
                if ($form) {
                    $programIds = [(string) $currentProgramId];
                    $formIdsArray = [(string) $form->id];
                    $openPrograms = collect([$currentProgram]);
                } else {
                    $programIds = [];
                    $formIdsArray = [];
                    $openPrograms = collect();
        ...
        // Get Track and Subtrack examples for the current program only
        $trackExamples = [];
        $subTrackExamples = [];

        if ($currentProgramId) {
            // Get tracks for the current program
            $tracks = Track::where('program_id', $currentProgramId)
                ->orderBy('order')
                ->get();
            ...
            // Get subtracks for the current program
            $subTracks = SubTrack::whereHas('track', function ($query) use ($currentProgramId) {
        ...
        $columns = [
            ImportColumn::make('program_id')
                ->label('Program ID')
                ->requiredMapping()
                ->examples($programIds ?: ['1', '2', '3'])
                ->rules(['required', 'integer', 'exists:programs,id']),
        ...
        // Collect all unique form fields from all open programs
        ...
        // Check if program has tracks/sub_tracks (even if not in form)
        $programHasTracks = !empty($trackExamples);
        $programHasSubTracks = !empty($subTrackExamples);

        // Add track and sub_track columns if:
        // 1. They exist in the form fields, OR
        // 2. The program has tracks/sub_tracks
        if (($hasTrackField || $programHasTracks) && !$hasTrackField) {
            // Add track column if program has tracks but not in form
        ...
        if (($hasSubTrackField || $programHasSubTracks) && !$hasSubTrackField) {
            // Add sub_track column if program has sub_tracks but not in form
        ...
        $excludedSlugs = ['email', 'program_id', 'form_id', 'participant_name'];

        // If track/sub_track are in form fields, they'll be added automatically
        // If they're not in form but program has them, we already added them above
        if ($hasTrackField) {
            $excludedSlugs[] = 'track';
        }
        if ($hasSubTrackField) {
    ...
    public function resolveRecord(): ?ProgramApplication
    {
        ProgramApplication::flushEventListeners();
        ...
        // Get the registration form - use form_id if provided, otherwise find by program_id
        if ($formId) {
            ...
            $form = Form::where('program_id', $programId)
                ->registrationType()
                ->published()
                ->active()
                ->first();
        ...
        if (!$form) {
            ...
            throw ImportValidationException::withMessages([
                'form_id' => "No registration form found for program ID: {$programId}" . ($formId ? " and form ID: {$formId}" : ""),
            ]);
        }
        ...
        $existingApplication = ProgramApplication::where('form_id', $form->id)
        ...
        if ($existingApplication) {
            ...
            throw ImportValidationException::withMessages([
                'email' => "Application already exists for this participant (email: {$email}) in this program",
            ]);
        }
        ...
        if (isset($this->data['track']) && $this->data['track']) {
            ...
            $track = is_numeric($trackValue)
                ? Track::where('id', $trackValue)->where('program_id', $programId)->first()
                : Track::where('slug', $trackValue)->where('program_id', $programId)->first();

            if (!$track) {
                throw ImportValidationException::withMessages([
                    'track' => "The selected track is invalid or does not belong to program ID: {$programId}.",
                ]);
            }
        }
        ...
        if (isset($this->data['sub_track']) && $this->data['sub_track']) {
            ...
            if ($subTrackValue && is_numeric($subTrackValue)) {
                ...
                // First, try to find by ID with program check using whereHas
                $subTrack = SubTrack::where('id', $subTrackId)
                    ->whereHas('track', function ($query) use ($programId) {
                ...
                if (!$subTrack) {
                    ...
                    if ($subTrack) {
                        ...
                        } elseif ((int)$subTrack->track->program_id !== (int)$programId) {
                            $subTrack = null; // Subtrack belongs to different program
                        }
                    }
                }
                ...
                if (!$subTrack) {
                    $subTrack = SubTrack::whereHas('track', function ($query) use ($programId) {
                            $query->where('program_id', $programId);
                        })
                        ->get()
            ...
            if (!$subTrack) {
                throw ImportValidationException::withMessages([
                    'sub_track' => "The selected sub-track is invalid or does not belong to program ID: {$programId}.",
                ]);
            }
        }
        ...
        foreach ($formFields as $field) {
            ...
            if ($slug === 'track' || $slug === 'sub_track') {
                ...
                if ($value) {
                    ...
                    if ($slug === 'track') {
                        // Find track by slug or ID, and verify it belongs to the program
                        $track = null;
                        if (is_numeric($value)) {
                        ...
                        if ($track) {
                            ...
                            // If program_id is set but track not found, set to null to avoid invalid data
                            $value = null;
                        } else {
                            // If no program_id, try to find by slug/ID without program check
                        ...
                        // Find subtrack by slug or ID, and verify it belongs to a track in the program
                        $subTrack = null;
                        if (is_numeric($value)) {
                            // Cast to integer for proper ID matching
                        ...
                        if ($subTrack) {
                            ...
                            // If program_id is set but subtrack not found, set to null
                            $value = null;
                        } else {
                            // If no program_id, try to find by slug/ID without program check
                            if (is_numeric($value)) {
    ...
    {
        // Remove email field - it's used for participant lookup but shouldn't be saved to ProgramApplication
    ...
    public function afterCreate()
    {
        ...
        if ($this->record && $this->record->type === 'submission') {
            ...
            if ($participant) {
                $participant->notify(new ProgramRegistration($this->record));
            }
        }
    }
