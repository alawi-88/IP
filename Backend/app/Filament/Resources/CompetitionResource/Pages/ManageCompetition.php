...
class ManageCompetition extends Page implements HasForms
{
    use InteractsWithRecord;
    use InteractsWithForms;

    protected static string $resource = CompetitionResource::class;
    protected static string $view = 'filament.pages.manage-competition';
                ...
                Forms\Components\Section::make('Program Details')
                    ...
                    ->schema([
                        ...
                        Forms\Components\FileUpload::make('banner')
                            ->label('Banner Image')
                            ->image()
                            ->directory('competitions/banners')
                            ->maxSize(5120),
    ...
    public function saveRegistration(): void
    {
        ...
        $data['competition_id'] = $this->record->id;
    ...
    protected function fillSubmissionEvalForm(): void
    {
        ...
        // Load project form config (first one for this competition)
        $projectConfig = ProjectFormConfig::whereHas('form', function ($q) {
            $q->where('competition_id', $this->record->id);
        })->first();
    ...
    {
        $projectForms = Form::where('competition_id', $this->record->id)
        ...
        $evalForms = Form::where('competition_id', $this->record->id)
                        ...
                        Forms\Components\Select::make('form_id')
                            ...
                            ->options(function (callable $get) {
                                ...
                                return Form::where('type', $formType)
                                    ->where('competition_id', $this->record->id)
                                    ->active()
                                    ->where('is_archived', false)
                                    ->get()
                                    ->mapWithKeys(function ($form) {
    ...
    {
        $evalForm = RegistrationEvaluationForm::where('competition_id', $this->record->id)->first();
        if ($evalForm) {
    ...
    protected function fillLabelsForm(): void
    {
        ...
        if ($this->record->labels()->count() === 0) {
            CompetitionLabel::seedDefaults($this->record->id);
            $this->record->refresh();
        }
    ...
    public function saveLabels(): void
    {
        ...
        foreach ($labels as $label) {
            if (!empty($label['id']) && in_array($label['id'], $existingIds)) {
                CompetitionLabel::where('id', $label['id'])->update([
                    'key' => $label['key'],
    ...
    {
        return Form::where('competition_id', $this->record->id)
            ->where('is_archived', false)
            ->orderBy('type')
            ->orderBy('id')
            ->get();
