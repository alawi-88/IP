...
use App\Models\SupervisorCompetition;
use App\Models\User;
...
class ListCompetitions extends ListRecords
{
    ...
    public function table(Table $table): Table
    {
        ...
        if ($query) {
            $table->query(Competition::query());
        } else {
            $supervisorCompetitions = UserCompetition::where('user_id', $user->id)->get();
            $competitionIds = $supervisorCompetitions->pluck('competition_id')->toArray();
            $table->query(Competition::query()->whereIn('id', $competitionIds));
        }
        ...
        return $table
                ...
                Tables\Actions\Action::make('archive')
                    ...
                    ->action(function ($record) {
                        ...
                        $result = $approvalService->processAction(
                            'archive',
                            ['is_archived' => true, 'competition_id' => $record->id, 'title' => $record->title],
                            $record->id,
                    ...
                    ->visible(fn($record) => !$record->isArchived() && CompetitionResource::canArchive($record) && !$record->isCurrent()),
            ...
            ->bulkActions([
                    ...
                    Tables\Actions\BulkAction::make('delete')
                        ...
                        ->action(function (Collection $records) {
                            ...
                            foreach ($records as $record) {
                                $result = $approvalService->processAction(
                                    'delete',
                                    ['competition_id' => $record->id, 'title' => $record->title],
                                    $record->id,
                ...
                Tables\Actions\ExportBulkAction::make()->exporter(CompetitionExporter::class)
                    ->columnMapping(false)
                    ->fileName('Competitions_List_' . now()->format('Y-m-d'))
    ...
    public function getTabs(): array
    {
        ...
        if ($user->isSuperAdmin()) {
            $baseQuery = Competition::query();
        } else {
            $competitionIds = UserCompetition::where('user_id', $user->id)
                ->pluck('competition_id')
                ->toArray();

            $baseQuery = Competition::query()->whereIn('id', $competitionIds);
        }
