...
namespace App\Filament\Resources\CompetitionApplicationResource\Pages;

use App\Filament\Exports\CompetitionApplicationExporter;
use App\Filament\Imports\CompetitionApplicationImporter;
use App\Filament\Imports\DynamicCompetitionApplicationImporter;
use App\Filament\Resources\CompetitionApplicationResource;
...
class ListCompetitionApplications extends ListRecords
{
    protected static string $resource = CompetitionApplicationResource::class;
    ...
    public function table(Table $table): Table
    {
        ...
        // Build base query
        $baseQuery = CompetitionApplication::query()->submission();
        ...
        if ($user && !$user->isSuperAdmin()) {
            ...
            // Super admin sees all applications (use byCompetition if needed)
            $baseQuery = CompetitionApplication::byCompetition()->submission();
        }
        ...
        return $table
            ...
            ->headerActions([
                Tables\Actions\ImportAction::make()
                    ...
                    ->importer(DynamicCompetitionApplicationImporter::class)
                    ->icon('heroicon-o-arrow-up-tray')
                    ->visible(function () {
                        if (!auth()->user()->can('create CompetitionApplication')) {
                            return false;
                        }

                        // Check if current competition exists, is open, and current stage is Registration
                        ...
                        $currentCompetition = \App\Models\Competition::published()
                            ->active()
                        ...
                        $currentStage = $currentCompetition->currentStage();
                        if (!$currentStage || $currentStage->slug !== 'registration') {
                            return false;
                        }

                        // Check if form exists for this competition
                        $form = \App\Models\Form::where('competition_id', $currentCompetitionId)
                            ->registrationType()
                            ->published()
                            ->active()
                            ->first();
            ...
            ->actions([
                Action::make('approve')
                    ...
                    ->authorize(fn ($record) =>
                        ...
                        CompetitionApplicationResource::canView($record)
                    ),
                ...
                Action::make('archive')
                    ...
                    ->visible(fn($record) => !$record->isArchived() && CompetitionApplicationResource::canArchive($record)),
                ...
                Action::make('restore')
                    ...
                    ->visible(fn($record) => $record->isArchived() && CompetitionApplicationResource::canRestore($record)),
            ...
            ->bulkActions([
                ...
                ExportBulkAction::make()->exporter(CompetitionApplicationExporter::class)
                    ->columnMapping(false)
    ...
    public function getTabs(): array
    {
        ...
        if (auth()->user()?->can('archive CompetitionApplication') || auth()->user()?->can('restore CompetitionApplication')) {
