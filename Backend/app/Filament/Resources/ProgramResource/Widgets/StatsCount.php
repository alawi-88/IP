...
class StatsCount extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $programs = $this->applyDateFilter(Program::query(), $from, $to)->get();
        $allProgramsCount = $programs->count();
        $openProgramsCount = $programs->filter(function ($program) {
            $registrationStage = $program->registrationStage();
            return $registrationStage && $registrationStage->ends_at && $registrationStage->ends_at->isFuture();
        })->count();
        
        $closedProgramsCount = $programs->filter(function ($program) {
            $registrationStage = $program->registrationStage();
        ...
        $applications = $this->applyDateFilter(ProgramApplication::query(), $from, $to)->submission()->active()->get();
        ...
        return [
            Stat::make('All Programs', $allProgramsCount),
            Stat::make('Open Programs', $openProgramsCount),
            Stat::make('Closed Programs', $closedProgramsCount),
