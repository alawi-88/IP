...
class EditProgram extends EditRecord
{
    ...
    protected function resolveRecord(string|int $key): \Illuminate\Database\Eloquent\Model
    {
        ...
        if (!ProgramResource::canView($record) && !ProgramResource::canEdit($record)) {
    ...
    protected function fillForm(): void
    {
        ...
        if ($this->record->isArchived() || !ProgramResource::canEdit($this->record)) {
            $this->form->disabled();
        }
    }
        ...
        if ($result['success']) {
            if ($result['requires_approval']) {
                Notification::make()
                    ...
                    ->body('Your program update request has been submitted for approval.')
                    ->success()
                    ->send();
                ...
                Notification::make()
                    ->title('Program Updated Successfully / تم تحديث المسابقة بنجاح')
