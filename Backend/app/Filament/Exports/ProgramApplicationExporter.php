...
class ProgramApplicationExporter extends BaseExporter
{
    protected static ?string $model = ProgramApplication::class;

    /**
     * Get registration form fields to use as dynamic export columns.
     * Uses current program from session when set; otherwise all forms that have applications.
     */
    protected static function getFormFieldsForExport(): \Illuminate\Support\Collection
    {
        ...
        $formIds = ProgramApplication::query()
            ->select('form_id')
            ->distinct()
     ...
     */
    protected static function formatFormSubmissionValue(ProgramApplication $record, string $slug, ?FormField $formField): string
    {
