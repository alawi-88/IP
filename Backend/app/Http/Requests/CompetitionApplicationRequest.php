...
use App\Services\CompetitionApplication;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Contracts\Validation\Validator;
use Illuminate\Http\Exceptions\HttpResponseException;

class CompetitionApplicationRequest extends FormRequest
{
    /**
    ...
    {
        return (new CompetitionApplication())->getMyApplications()->where('competition_id', request('competition_id'))->isEmpty();
    }

    protected function failedAuthorization(): void
    {
        abort(422, __('competition_application.already_submitted'));
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            foreach ($answers as $key => $value) {
                    ...
                    foreach ($fieldValidationRules as $rule) {
                        ...
                        if ($ruleName === 'min' && $ruleValue !== null) {
                            ...
                            if (mb_strlen($value, 'UTF-8') < $minLength) {
                                ...
                                $validator->errors()->add(
                                    "answers.{$key}",
                                    __('competition_application.The field :field must be at least :min characters.', ['field' => $fieldLabel, 'min' => $minLength])
                                );
                            }
                        }
                        ...
                        if ($ruleName === 'max' && $ruleValue !== null) {
                            ...
                            if (mb_strlen($value, 'UTF-8') > $maxLength) {
                                ...
                                $validator->errors()->add(
                                    "answers.{$key}",
                                    __('competition_application.The field :field must not exceed :max characters.', ['field' => $fieldLabel, 'max' => $maxLength])
                                );
                            }
                        }
                    }
                ...
                if (is_array($value)) {
                    ...
                    if (count($value) > $maxArraySize) {
                        ...
                        $validator->errors()->add(
                            "answers.{$key}",
                            __('competition_application.The field :field must not contain more than :max items.', ['field' => $fieldLabel, 'max' => $maxArraySize])
                        );
                    }
                        ...
                        if (is_string($nestedValue)) {
                            ...
                            if (mb_strlen($nestedValue, 'UTF-8') > $maxInputLength) {
                                ...
                                $validator->errors()->add(
                                    ...
                                    __('competition_application.The field :field must not exceed :max characters.', ['field' => $fieldLabel . '.' . $nestedKey, 'max' => $maxInputLength])
                                );
                            }
                        }
                    }
            ...
            if ($totalSize > $maxTotalSize) {
                $validator->errors()->add(
                    'answers',
                    __('competition_application.The total size of all form fields must not exceed 1MB. Please reduce the length of your input.')
                );
            }
            ...
            // Get the registration configuration for this competition
            $registrationConfig = RegistrationFormConfig::where('competition_id', $competitionId)
            ...
            if ($registrationConfig->registration_type === 'team' && !$hasTeam) {
                $validator->errors()->add(
                    'answers.has_team',
                    __('competition_application.This program only allows team registration. You must register as a team.')
                );
            }
            ...
            if ($registrationConfig->registration_type === 'individual' && $hasTeam) {
                $validator->errors()->add(
                    'answers.has_team',
                    __('competition_application.This program only allows individual registration. Team registration is not allowed.')
                );
            }
            ...
            if (in_array($registrationConfig->registration_type, ['team', 'both']) && $hasTeam) {
                ...
                $teamFormConfig = TeamFormConfig::where('competition_id', $competitionId)
                    ->active()
                    ->notArchived()
                    ->first();
    ...
    public function messages(): array
    {
        return [
            // Step 1 Errors
            'answers.team_logo.image' => __('competition_application.The team logo must be a valid image.'),
            'answers.team_logo.max' => __('competition_application.Image size must not exceed 1MB.'),

            'answers.team_strength.required_if' => __('competition_application.Please describe your team\'s strengths.'),
            'answers.team_strength.max' => __('competition_application.The maximum message limit is 500 characters.'),

            'answers.team_serial.min' => __('competition_application.At least one member must be added to the team.'),
            'answers.team_serial.required_if' => __('competition_application.At least one member must be added to the team.'),
            'answers.team_serial.max' => __('competition_application.The maximum number of team members is 6.'),

            // Step 2 Errors
            'answers.track_id.exists' => __('competition_application.Please select a valid Track.'),
            'answers.sub_track_id.exists' => __('competition_application.Please select a valid challenge.'),
        ];
