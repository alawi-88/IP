...
class CompatibleSanctumAuth
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (RateLimiter::tooManyAttempts($key, 20)) { // Increased to 20 attempts
        ...
        RateLimiter::hit($key, 60); // 20 attempts per minute
        ...
        if (str_contains($token, '|')) {
            ...
            if (!hash_equals($tokenRecord->token, hash('sha256', $tokenHash))) {
            ...
            // Try to find by plain text match (fallback for legacy tokens)
