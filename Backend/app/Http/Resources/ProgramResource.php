...
class ProgramResource extends JsonResource
{
    ...
    {
        $forms = Form::where('program_id', $this->id)->pluck('id');

        $project = $this->projectId ? Project::find($this->projectId) : null;

        $programApplications = $this->applicationId
            ? ProgramApplication::find($this->applicationId)
            : ProgramApplication::whereIn('form_id', $forms)->latest()->first();

        $selectedTrackId = $project?->form_submissions->track ?? $programApplications?->form_submissions->track;
        $selectedSubTrackId = $project?->form_submissions->sub_track  ?? $programApplications?->form_submissions->sub_track;

        // $selectedTrackId = $programApplications?->form_submissions->track;
        // $selectedSubTrackId = $programApplications?->form_submissions->sub_track;
        $type = null;
        ...
        if ($currentStage) {
            ...
            if ($programApplications) {
                $stageProjects = Project::where('application_id', $programApplications->id)
        ...
        return [
            ...
            'branding' => BrandingProgram::getByProgramId($this->id) ?? null,
        ];
    }
}
