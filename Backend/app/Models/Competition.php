...
class Competition extends Model
{
    ...
    protected string $moduleName = 'Competition';
    protected string $logName = 'competition';
    ...
    protected static function boot()
    {
        ...
        static::created(function (Competition $competition) {
            if (auth()->check()) {
                $competition->users()->syncWithoutDetaching([auth()->id()]);
            }
            
            // Fire CompetitionCreated event
            event(new \App\Events\CompetitionCreated($competition));
        });

        static::deleting(function ($competition) {
            $committees = Committee::where('competition_id', $competition->id);
            $committeesIds = $committees->pluck('id');
            CommitteeJudge::whereIn('committee_id', $committeesIds)->delete();
            $committees->delete();

            $forms = Form::where('competition_id', $competition->id)->get();
    ...
    {
        return $this->belongsToMany(Judge::class, 'competition_judge')->withTimestamps();
    }
    ...
    {
        return $this->belongsToMany(Mentor::class, 'mentor_competitions')->withTimestamps();
    }
    ...
    {
        return $this->hasMany(CompetitionTab::class);
    }
    ...
    {
        return (int) $this->id === (int) session('current_competition_id');
    }
    ...
    {
        session()->put('current_competition_id', $this->id);
    }
    ...
    /**
     * Check if the currently authenticated user can access (edit or delete) this program (competition)
     * This checks if the user is assigned to this competition or is a super admin.
     * 
    ...
    public function canAccessProgram(?int $userId = null): bool
    {
        ...
        // Check if user is assigned to this competition (through UserCompetition relation/table)
        if (
            \App\Models\UserCompetition::where('user_id', $user->id)
    ...
    public static function columns(): array
    {
        return [
            ...
            Tables\Columns\TextColumn::make('last_updated_by')
                ...
                ->getStateUsing(function ($record) {
                    $activity = \App\Models\ActivityLog::where('log_name', 'competition')
