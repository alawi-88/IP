...
readonly class QualifiedMemberApplication implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (strlen($value) !== 8) {
            $fail(__('program_application.The member serial number must be 6 digits.'));
            return;
        }
        ...
        if (ProgramApplication::where('participant_id', $participant->id)
            ->where('program_id', request('application_id') ? getProgramId(request('application_id')) : request('program_id'))
            ->exists()) {
            $fail(__('program_application.The member registered in the program', ['participant' => $participant->serial_number]));
            return;
        }
        ...
        if ($participant && TeamMember::where('participant_id', $participant->id)
                ->whereHas('team', fn ($query) => $query->where('is_archived', false)
                    ->whereHas('application', fn ($query) => $query->where('program_id', request('program_id'))))
                ->exists()) {
