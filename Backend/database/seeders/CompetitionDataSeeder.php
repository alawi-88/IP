...
class CompetitionDataSeeder extends Seeder
{
    public function run(): void
    {
        $this->command->info('Seeding competition data...');

        // ─── Competition 1: Saudi Innovation Hackathon 2025 ───
        // Already has: reg config (both), team config, eval config (1 stage), project config, AI scoring, eval form
        // Just needs labels
        $this->seedLabelsFor(1);
        $this->command->info('  ✓ Competition 1: Labels seeded');

        // ─── Competition 2: TGA Regulatory Sandbox 2026 ───
        // Has: reg config (individual), project configs (forms 11-12)
        // Needs: eval form, eval config (2 stages), team config, AI scoring, labels
        $this->seedCompetition2();

        // ─── Competition 3: Transportation Sandbox 2025 ───
        ...
        $this->seedCompetition3();

        // ─── Competition 5: Naql Thon 5 ───
        ...
        $this->seedCompetition5();

        $this->command->info('Done seeding all competition data.');
    }

    private function seedCompetition2(): void
    {
        ...
        $evalForm = Form::firstOrCreate(
            ['competition_id' => 2, 'type' => 'evaluation', 'name->en' => 'Regulatory Compliance Evaluation'],
            [
                'competition_id' => 2,
                'type' => 'evaluation',
        ...
        $this->command->info('  ✓ Competition 2: Eval form, 2-stage config, team config, labels seeded');
    }

    private function seedCompetition3(): void
    {
        $comp = Competition::find(3);
        if (!$comp) return;

        // Create evaluation forms (2 different ones for multi-stage)
        $evalForm1 = Form::firstOrCreate(
            ['competition_id' => 3, 'type' => 'evaluation', 'name->en' => 'Monthly Performance Review'],
            [
                'competition_id' => 3,
                'type' => 'evaluation',
        ...
        $evalForm2 = Form::firstOrCreate(
            ['competition_id' => 3, 'type' => 'evaluation', 'name->en' => 'Final Sandbox Assessment'],
            [
                'competition_id' => 3,
                'type' => 'evaluation',
        ...
        $this->command->info('  ✓ Competition 3: 2 eval forms, 3-stage config, labels seeded');
    }

    private function seedCompetition5(): void
    {
        $comp = Competition::find(5);
        if (!$comp) return;

        // Create evaluation forms (2 different stages: screening + final)
        $screeningForm = Form::firstOrCreate(
            ['competition_id' => 5, 'type' => 'evaluation', 'name->en' => 'Naql Thon 5 Screening Evaluation'],
            [
                'competition_id' => 5,
                'type' => 'evaluation',
        ...
        $finalForm = Form::firstOrCreate(
            ['competition_id' => 5, 'type' => 'evaluation', 'name->en' => 'Naql Thon 5 Final Judging'],
            [
                'competition_id' => 5,
                'type' => 'evaluation',
        ...
        $this->command->info('  ✓ Competition 5: 2 eval forms, 2-stage config, labels seeded');
    }

    private function seedLabelsFor(int $competitionId): void
    {
        CompetitionLabel::seedDefaults($competitionId);
    }
}
