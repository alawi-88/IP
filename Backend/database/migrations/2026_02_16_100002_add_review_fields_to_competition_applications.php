...
return new class extends Migration
{
    ...
    public function up(): void
    {
            ...
            if (!Schema::hasColumn('competition_applications', 'final_evaluation_score')) { $table->decimal('final_evaluation_score', 8, 2)->nullable() }
                
                ->comment('Sum of all evaluator scores for registration');
            if (!Schema::hasColumn('competition_applications', 'minimum_score_threshold')) { $table->unsignedInteger('minimum_score_threshold')->nullable() }
                
                ->comment('Minimum score required for this program');

            // Review & Decision fields (IN-2030)
            if (!Schema::hasColumn('competition_applications', 'decision_reason')) { $table->text('decision_reason')->nullable() }
                
                ->comment('Reason for approval/rejection');
            if (!Schema::hasColumn('competition_applications', 'editable_fields')) { $table->json('editable_fields')->nullable() }
                
                ->comment('JSON array of field slugs admin has approved for editing');
            if (!Schema::hasColumn('competition_applications', 'edit_notes')) { $table->json('edit_notes')->nullable() }
                
                ->comment('JSON object: field_slug => admin notes for each field');
            if (!Schema::hasColumn('competition_applications', 'edit_requested_at')) { $table->timestamp('edit_requested_at')->nullable() }
                ;
            if (!Schema::hasColumn('competition_applications', 'resubmitted_at')) { $table->timestamp('resubmitted_at')->nullable() }
                ;
            if (!Schema::hasColumn('competition_applications', 'reviewed_by')) { $table->foreignId('reviewed_by')->nullable() }
                
                ->constrained('users')
                ->nullOnDelete();
            if (!Schema::hasColumn('competition_applications', 'reviewed_at')) { $table->timestamp('reviewed_at')->nullable() }
                ;
        });
        }
        ...
        if (Schema::hasTable('registration_form_configs')) {
            ...
            if (!Schema::hasColumn('registration_form_configs', 'minimum_score_threshold')) { $table->unsignedInteger('minimum_score_threshold')->nullable() }
