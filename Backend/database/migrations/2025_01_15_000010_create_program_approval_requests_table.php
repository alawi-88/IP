...
class CreateProgramApprovalRequestsTable extends Migration
{
    public function up()
    {
        Schema::create('program_approval_requests', function (Blueprint $table) {
            ...
            $table->foreignId('program_id')->nullable()
                ->constrained('competitions', 'id', 'par_program_id_foreign')
                ->onDelete('cascade');

            $table->enum('action_type', ['create', 'update', 'delete', 'archive']);

            $table->foreignId('requested_by')
                ->constrained('users', 'id', 'par_requested_by_foreign')
                ->onDelete('cascade');

            $table->foreignId('approval_workflow_id')
                ->constrained('approval_workflows', 'id', 'par_workflow_id_foreign')
                ->onDelete('cascade');
