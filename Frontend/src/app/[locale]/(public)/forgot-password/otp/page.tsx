...
export default function ForgotPasswordOTPPage() {
  ...
  const { mutate: confirmOTP, isPending: confirmOTPisPending } = useMutation({
    mutationFn: async (data: { code: string }) => {
      const response = await axiosInstance.post(
        `/participants/check-password-reset-code`,
        data
      );
    ...
    onSuccess: (data: { token?: string; code_exists: boolean }) => {
      ...
      if (data.code_exists) {
        ...
        sessionStorage.setItem("participant_otp_rest_password", code);
        sessionStorage.removeItem("participant_reset_password_email");
      } else {
  ...
  const email = sessionStorage.getItem("participant_reset_password_email") || "";
