...
export default function NotificationsPage() {
  ...
  const { data: notifications, isLoading } = useQuery<Notification[]>({
    ...
    queryFn: async () => {
      const response = await axiosInstance.get(`/judges/notifications`);
  ...
  const { mutate: markAllAsRead } = useMutation({
    mutationFn: async () => {
      await axiosInstance.post(`/judges/notifications/mark-all-as-read`);
    },
  });
  ...
  useEffect(() => {
    markAllAsRead();
    queryClient.invalidateQueries({ queryKey: ["judge-notifications-count"] });
  }, []);
