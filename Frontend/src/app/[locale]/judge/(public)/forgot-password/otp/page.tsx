...
export default function ForgotPasswordOTPPage() {
  ...
  const { mutate: confirmOTP, isPending: confirmOTPisPending } = useMutation({
    mutationFn: async (data: { code: string }) => {
      const response = await axiosInstance.post(
        `/judges/check-password-reset-code`,
        data
      );
    ...
    onSuccess: (data: { token?: string; code_exists: boolean }) => {
      ...
      if (data.code_exists) {
        router.push(`/judge/forgot-password/reset`);
        sessionStorage.setItem("judge_otp_rest_password", code);
        sessionStorage.removeItem("judge_reset_password_email");
      } else {
  ...
  const email = sessionStorage.getItem("judge_reset_password_email") || "";
          ...
          <div className="flex flex-col gap-y-3 items-center mx-auto mt-4">
            ...
            <p className="text-primary-green-900 font-medium">
              ...
              <Button
                ...
                loading={resendOTPisPending || recaptchaTokenLoading}
              >
                <div>
                  {t("resend-code")}
                </div>
