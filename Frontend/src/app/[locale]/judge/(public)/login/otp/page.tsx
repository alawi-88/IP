...
import { Judge } from "@/lib/interfaces";
...
export default function ForgotPasswordOTPPage() {
  ...
  const { mutate: resendOTP, isPending: resendOTPisPending } = useMutation({
    mutationFn: async (data: any) => {
      const response = await axiosInstance.post(`/judges/resend-otp`, data);
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data: APIResponse) => {
      ...
      sessionStorage.removeItem("judge_otp_email");
      sessionStorage.removeItem("judge_otp_password");
      sessionStorage.removeItem("judge_rememberMe");
      sessionStorage.removeItem("judge_registerEmail");
      setJudge({ ...data.judge });
  ...
  const email = sessionStorage.getItem("judge_otp_email") || "";
  const password = sessionStorage.getItem("judge_otp_password") || "";
  const rememberMe = sessionStorage.getItem("judge_rememberMe") || 0;
