...
export default function ForgotPasswordOTPPage() {
  ...
  const setMentor = useUserStore((state) => state.loginMentor);

  const router = useRouter();

  const { mutate: resendOTP, isPending: resendOTPisPending } = useMutation({
    mutationFn: async (data: any) => {
      const response = await axiosInstance.post(`/mentors/resend-otp`, data);
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data: APIResponse) => {
      sessionStorage.removeItem("mentor_otp_email");
      sessionStorage.removeItem("mentor_otp_password");
      sessionStorage.removeItem("mentor_rememberMe");
      sessionStorage.removeItem("mentor_registerEmail");
  ...
  const email = sessionStorage.getItem("mentor_otp_email") || "";
  const password = sessionStorage.getItem("mentor_otp_password") || "";
  const rememberMe = sessionStorage.getItem("mentor_rememberMe") || 0;
  ...
  const onResendOTP = async () => {
    ...
    try {
      ...
      resendOTP({ email, "g-recaptcha-response": token });
    } catch (err) {
