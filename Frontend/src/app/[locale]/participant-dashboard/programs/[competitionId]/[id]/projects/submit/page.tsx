...
import { MdOutlineLink, MdOutlineInsertDriveFile } from "react-icons/md";
...
import { DynamicForm, Field, Team } from "@/lib/interfaces";
...
export default function ProjectSubmit() {
  ...
  const { data: dynamicForm, isLoading: isDynamicFormLoading } =
    useQuery<DynamicForm>({
      queryKey: ["projectForm", competitionId, draftProjectId, formId],
      enabled: !!competitionId,
      queryFn: async () => {
        try {
          const response = await axiosInstance.get(`/forms/projects/form`, {
            params: {
              form_id: formId,
              ...(draftProjectId
                ? {
  ...
  const { data: formConfig, isLoading: isFormConfigLoading } = useQuery({
    queryKey: ["projectConfig", competitionId],
    enabled: !!competitionId,
    queryFn: async () => {
      try {
        const response = await axiosInstance.get(
          "/forms/projects-form-config",
          {
            params: {
              form_id: formId,
              ...(draftProjectId
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (values: any) => {
      ...
      formData.append(
        "stage_id",
        String(dynamicForm?.competition?.current_stage_id)
      );
      ...
      const response = await axiosInstance.post(
        `/participants/projects?application_id=${id}`,
        formData,
        {
          headers: {
    ...
    onSuccess: (_, ...variables) => {
      if (isDraft) {
        ...
        router.push(
          `/participant-dashboard/my-competitions/${competitionId}/${id}/projects`
        );
      } else {
  ...
  const { mutate: deleteDraft, isPending: isDeleteDraftPending } = useMutation({
    mutationFn: async (_) => {
      const formData = new FormData();
      formData.append("project_id", String(draftProjectId));
      formData.append("application_id", String(id));
      const response = await axiosInstance.post(
        "/participants/projects/reset-draft",
        formData
      );
      return response.data;
    },
    onSuccess: (response) => {
      ...
      setAiSuggestions({});
      queryClient.invalidateQueries({
        queryKey: ["projectForm", competitionId, draftProjectId],
      });
      queryClient.invalidateQueries({
        queryKey: ["projects", id],
        type: "all",
      ...
      router.replace(
        `/participant-dashboard/my-competitions/${competitionId}/${id}/projects/submit?${params.toString()}`
      );
    },
    onError: (error: APIError) => {
      if (error.response.data.message) {
    ...
    try {
      ...
      if (allSteps.length && !isDraft) {
        ...
        if (firstErrorStep !== -1) {
          ...
          if (firstErrorStep === 0) {
            fieldNames.push("project_name");
            if (dynamicForm?.competition?.tracks?.length) {
  ...
  function getSubTracks(selectedTrackId: any) {
    const selectedTrack = dynamicForm?.competition?.tracks.find(
  ...
  // set selected tracks form team or competition
  useEffect(() => {
    if (dynamicForm?.id) {
      const selectedTrack = dynamicForm.competition?.tracks.find(
        (track) => track.is_selected
      );
      if (team?.track?.id || selectedTrack) {
        getSubTracks(team?.track?.id || selectedTrack?.id);
        setLastDraftValues((prev) => ({
          ...prev,
          track: team?.track?.id || selectedTrack?.id,
          sub_track:
            team?.sub_track?.id ||
            selectedTrack?.sub_tracks.find((sub_track) => sub_track.is_selected)
              ?.id,
        }));
      }
    }
  ...
  useEffect(() => {
    ...
    if (dynamicForm?.submit_type === "draft") {
      ...
      setLastDraftValues((prev) => ({
        ...prev,
        project_name: dynamicForm?.metadata?.project_name,
      }));
    ...
    <>
      ...
      <div className="dashboard-card">
        <h1 className="text-2xl text-primary-900 font-bold">
          <span className="me-2">{t("submit-project")}</span>
                        ...
                        <>
                          ...
                          >
                            <Select
                              ...
                              showSearch={true}
                              allowClear={true}
                              options={dynamicForm.competition.tracks.map(
                                (track) => ({
                                  label: track.name,
                                  value: track.id,
                                })
                              ...
                              disabled={!formConfig?.allow_track_change}
                            />
                          </Form.Item>
                          <Form.Item
                            label={t("sub-track")}
                ...
                <div className="flex justify-between items-center">
                  <div className="flex gap-4 flex-wrap">
                    ...
                    {dynamicForm.ai_enhancement?.enabled && (
                      ...
                      >
                        <PiStarFourFill className="text-primary" size={24} />
                        {t("ai.ai-enhance")}
                      </Button>
                    )}
                  </div>
            ...
            {dynamicForm?.steps?.length > 0 &&
              dynamicForm?.steps[currentStep]?.fields?.length > 0 && (
                <>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex gap-4 flex-wrap">
                      {dynamicForm?.steps?.length > 1 && currentStep !== 0 ? (
                        <Button
                          type="default"
                          htmlType="button"
                          size="large"
        ...
        <FeedbackModal
          ...
          title={t("project-submitted-successfully")}
          type="success"
          onBtnClick={() => {
            queryClient.invalidateQueries({
              queryKey: ["projects", id],
        ...
        >
          <div className="relative w-full h-full sm:max-h-[95vh] overflow-y-auto">
            <div className="relative text-center py-10 px-6 gap-y-8 sm:gap-y-10 bg-card flex flex-col items-center">
              <h3 className="font-semibold text-xl">{t("draft.confirm")}</h3>
