...
export default function ValidationPage() {
  ...
  const { data: page, isLoading } = useQuery({
    queryKey: ["startup", startupId, "path-to-mvp", "validation"],
    queryFn: () =>
      startupApi.getVaPage(startupId, "path-to-mvp", "validation"),
    onSuccess: (data) => {
  ...
  const completeMutation = useMutation({
    mutationFn: () =>
      startupApi.completeVaPage(startupId, "path-to-mvp", "validation"),
    onSuccess: () => {
      setIsCompleted(true);
  ...
  return (
    <VaPageLayout
      title={t("va.validation", "validation")}
      breadcrumbs={[
