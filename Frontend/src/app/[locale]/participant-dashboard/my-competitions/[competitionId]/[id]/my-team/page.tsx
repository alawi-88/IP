...
import { MyCompetition, Team } from "@/lib/interfaces";
...
export default function YourTeamPage() {
  ...
  const { data: formConfig, isLoading: isFormConfigLoading } = useQuery({
    queryKey: ["teamConfig", competitionId],
  ...
  const isTeamFormation =
    myApplication?.competition?.current_stage_slug === "team-formation";
        ...
        {isTeamFormation && (
          <div className="flex justify-center flex-wrap gap-4">
            <AddTeamModal
              ...
              competition={myApplication?.competition}
              formConfig={formConfig}
              refetch={refetch}
            />
            <Link
              href={`/participant-dashboard/my-competitions/${competitionId}/${id}/teams`}
            >
