...
export default function MyCompetitionLayout({
  children,
}: {
  ...
  const { competitionId, id, eventId, teamId, projectId } = useParams<{
  ...
  const { data: activeTabs, isLoading } = useQuery<Tab[]>({
    queryKey: ["participants/competition-tabs"],
    queryFn: async () => {
      const response = await axiosInstance.get(
        `/participants/competition-tabs`,
        {
          params: {
  ...
  useEffect(() => {
    const brandingTheme = myApplication?.competition?.branding;
  ...
  const breadcrumbItems: BreadcrumbItemType[] = [
    {
      title: t("my-competitions"),
      onClick: () => router.push("/participant-dashboard/my-competitions"),
      className: "cursor-pointer hover:text-primary",
    },
    {
      title: myApplication?.competition?.title
        ? myApplication.competition.title
        : "-",
    },
  ];

  // if (myCompetition?.competition?.title) {
  //   breadcrumbItems.push({
  //     title: myCompetition.competition.title,
  //     className: "cursor-pointer hover:text-primary",
  //     onClick: () =>
  //       router.push(
  //         `/participant-dashboard/my-competitions/${competitionId}/${id}`
  //       ),
  //   });
  // }
