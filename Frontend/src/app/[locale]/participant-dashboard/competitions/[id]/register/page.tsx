...
export default function Register() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (values: any) => {
      ...
      formData.append("competition_id", String(id));
      ...
      if (isDraft) {
        ...
        router.push(
          `/participant-dashboard${
            dynamicForm?.competition?.type &&
            dynamicForm.competition.type.slug !== programsTypes[0]
              ? `?program_type=${dynamicForm.competition.type.slug}`
              : ""
          }`
        );
      } else {
  ...
  const { mutate: deleteDraft, isPending: isDeleteDraftPending } = useMutation({
    mutationFn: async (_) => {
      ...
      const response = await axiosInstance.post(
        "/participants/competition-applications/reset-draft",
        formData
      );
      return response.data;
    },
  ...
  // if (dynamicForm?.competition?.is_closed) {
    ...
    <>
      ...
      <div className="dashboard-card">
        <h1 className="text-2xl text-primary-900 font-bold">
          <span className="me-2">{t("join-the-competition")}</span>
        ...
        {!isAgeValid ? (
          ...
          <>
            ...
            <FeedbackModal
              ...
              title={t("you-have-successfully-registered-for-the-competition")}
              subtitle={t("you-will-be-notified-of-your-joining-status-soon")}
              btnLabel={t("go-to-competitions")}
              type="success"
              onBtnClick={() => {
                queryClient.invalidateQueries({
