...
import { ProgramApplicationList } from "@/lib/interfaces";
...
export default function ProgramApplications() {
  ...
  const {
    ...
    isLoading,
    isRefetching,
  } = useQuery<ProgramApplicationList>({
    queryKey: ["my-programs", query, locale],
    queryFn: async () => {
      const params = { ...query };
      ...
      {!programs?.data?.length && !isLoading && !isRefetching ? (
        <Empty
          description={
            <span className="font-bold">
              {t("sorry-you-have-not-registered-for-a-program-yet")}
            </span>
          }
        >
          <Link href="/participant-dashboard">
            <Button type="primary">
              {t("discover-the-available-programs")}
            </Button>
          </Link>
        </Empty>
      ) : isRefetching ? (
        ...
        programs?.data.length && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {programs?.data?.map(({ id, program, status, created_at }) => (
              <div
                key={id}
                  ...
                  <div className="flex flex-col gap-y-3 p-6 bg-card rounded-lg w-full">
                    ...
                    {status === "approved" ? (
                      <div className="flex justify-between">
                        <Link
                          href={`/participant-dashboard/my-programs/${program.id}/${id}`}
                        >
                          <Button
                            type="primary"
                            size="small"
              ...
              <div className="z-10 w-full h-full flex flex-col justify-between">
                ...
                <div className="flex flex-col gap-y-3 p-6 bg-card rounded-lg w-full">
                  ...
                  {status === "approved" ? (
                    <div className="flex justify-between">
                      <Link
                        href={`/participant-dashboard/my-programs/${program.id}/${id}/evaluate`}
                      >
                        <Button
                          type="primary"
                          size="small"
