...
export default function ProjectSubmit() {
  ...
  const { data: dynamicForm, isLoading: isDynamicFormLoading } =
    useQuery<DynamicForm>({
      queryKey: ["projectForm", programId, draftProjectId, formId],
      enabled: !!programId,
      queryFn: async () => {
        try {
  ...
  const { data: formConfig, isLoading: isFormConfigLoading } = useQuery({
    queryKey: ["projectConfig", programId],
    enabled: !!programId,
    queryFn: async () => {
      try {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (values: any) => {
      ...
      formData.append(
        "stage_id",
        String(dynamicForm?.program?.current_stage_id)
      );
    ...
    onSuccess: (_, ...variables) => {
      if (isDraft) {
        ...
        router.push(
          `/participant-dashboard/my-programs/${programId}/${id}/projects`
        );
      } else {
  ...
  const { mutate: deleteDraft, isPending: isDeleteDraftPending } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setAiSuggestions({});
      queryClient.invalidateQueries({
        queryKey: ["projectForm", programId, draftProjectId],
      });
      queryClient.invalidateQueries({
        queryKey: ["projects", id],
        type: "all",
      ...
      router.replace(
        `/participant-dashboard/my-programs/${programId}/${id}/projects/submit?${params.toString()}`
      );
    },
    onError: (error: APIError) => {
      if (error.response.data.message) {
  ...
  // set selected tracks from team or program
  useEffect(() => {
    if (dynamicForm?.id) {
      const selectedTrack = dynamicForm.program?.tracks.find(
