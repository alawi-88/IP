...
import { Program, ProgramApplicationList, Stage, Team, ProgramApplication} from "@/lib/interfaces";
...
export default function ProjectPage() {
  ...
  const allStages = myApplication?.program?.stages || [];
  const currentStageId = myApplication?.program?.current_stage_id;
  const currentStage = myApplication?.program?.current_stage;
  ...
  useEffect(() => {
    if (!myApplication?.program?.stages?.length) return;

    // Default to the current active project stage, or the first project stage
    const activeProjectStage = myApplication.program.stages.find(
      (stage) =>
        stage.slug?.startsWith("project") &&
        stage.id === myApplication.program.current_stage_id
    );
    const firstProjectStage = myApplication.program.stages.find((stage) =>
      stage.slug?.startsWith("project")
    );
        ...
        >
          ...
          <div className="flex items-center max-sm:flex-wrap gap-4">
            ...
            <Link
              href={
                selectedStage?.id ===
                  myApplication?.program?.current_stage_id &&
                project.submit_type === "draft"
          ...
          <div className="flex items-center max-sm:flex-wrap gap-4">
            <Link
              href={`${
                myApplication?.program?.current_stage_id !==
                  selectedStage?.id ||
                !myApplication?.program?.current_stage?.ends_at ||
                !canSubmitProject
                  ? ``
